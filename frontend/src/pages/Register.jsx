            ...
            try {
                const response = await fetch('http://localhost:5000/api/auth/register', {
                    ...
                    body: JSON.stringify({
                        ...
                        password: formData.password, // Ideally, you'd handle passwords differently, but this matches the backend model
                        role: 'customer',
                ...
                if (!response.ok) {
                    ...
                    console.error('Backend sync failed:', errorData.message);
                    // We don't necessarily want to fail registration if DB sync fails, 
                ...
                console.error('Error syncing with backend:', syncErr);
            }
        ...
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 p-4">
            ...
            <div className="relative w-full max-w-md bg-white/10 backdrop-blur-xl rounded-2xl border border-white/20 shadow-2xl p-8 transform transition-all duration-300 hover:scale-[1.01]">
                <div className="text-center mb-8">
                    ...
                    <p className="text-blue-200">Start your financial journey today</p>
                </div>

                {error && (
                    <div className="mb-6 p-4 rounded-xl bg-red-500/20 border border-red-500/50 text-red-200 text-sm animate-shake">
                        {error}
                    </div>
                )}
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    <div className="space-y-2">
                        <label className="text-sm font-medium text-blue-100 flex items-center px-1">
                            <User className="w-4 h-4 mr-2 opacity-70" /> Name
                        </label>
                        <input
                            type="text"
                            name="name"
                    ...
                    <div className="space-y-2">
                        <label className="text-sm font-medium text-blue-100 flex items-center px-1">
                            <Mail className="w-4 h-4 mr-2 opacity-70" /> Email
                        </label>
                        <input
                            type="email"
                            name="email"
                    ...
                    <div className="space-y-2">
                        <label className="text-sm font-medium text-blue-100 flex items-center px-1">
                            <Phone className="w-4 h-4 mr-2 opacity-70" /> Phone
                        </label>
                        <input
                            type="tel"
                            name="phone"
